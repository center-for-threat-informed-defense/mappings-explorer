...
def build_official_datacomponent_names_from_xlsx(analytics_xlsx: Path) -> dict[str, str]:
    """Return lowercased data component name -> canonical ATT&CK display name."""
...
def build_event_id_to_datacomponents_from_stix(
    ...
    for analytic in mitre_attack_data.get_analytics(remove_revoked_deprecated=True):
        for log_ref in mitre_attack_data.get_field(
            analytic, "x_mitre_log_source_references", []
        ) or []:
            log_source = str(mitre_attack_data.get_field(log_ref, "name", "")).strip().lower()
...
def supplemental_datacomponents_for_event(
    ...
    data_components = set(SUPPLEMENTAL_EVENT_TO_DATACOMPONENT.get((attack_log_source, event_id), ()))
...
def normalize_row_datacomponents(
    ...
    if attack_log_source and event_id is not None and not (
        isinstance(event_id, float) and pd.isna(event_id)
    ):
        event_id_text = str(int(float(event_id)))
        data_components = set(
            context.event_id_to_datacomponents.get((attack_log_source, event_id_text), set())
        )
...
def get_technique_id(
    ...
    """
    Given a RelationshipEntry-like dict, return the technique ATT&CK ID (T#### or T####.###).
...
def build_datacomponent_to_analytic_ids(
    ...
    ATT&CK analytics link to data components through x_mitre_log_source_references entries, which
    may contain either:
...
def build_group_key_series(dataframe: pd.DataFrame, group_by: str) -> pd.Series:
    ...
    if group_by == "source_row":
        if SOURCE_ROW_INDEX_COLUMN not in dataframe.columns:
            raise ValueError(
                f"Missing {SOURCE_ROW_INDEX_COLUMN!r}; row grouping requires rows technique format"
            )
...
def find_consecutive_group_ranges(group_keys: list[str]) -> list[tuple[int, int]]:
    ...
    while index < len(group_keys):
        end_index = index + 1
        while end_index < len(group_keys) and group_keys[end_index] == group_keys[index]:
            end_index += 1
    ...
    *,
    ...
    for _, row in dataframe.iterrows():
        ...
        if normalization_context is not None:
            normalized_datacomponents, normalization_method = normalize_row_datacomponents(
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "ATT&CK analytics excel export (e.g., enterprise-attack-v19.1-analytics.xlsx). "
            "If provided with --attack-relationships-xlsx, the script will use the excel exports "
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "ATT&CK relationships excel export (e.g., enterprise-attack-v19.1-relationships.xlsx). "
    ...
    parser.add_argument(
        "--no-normalize",
        action="store_true",
        help="Disable normalization of event descriptions to official ATT&CK data components",
    )
    parser.add_argument(
        "--technique-format",
    ...
    parser.add_argument(
        ...
        default="event_id",
        help=(
            "For --technique-format rows, group technique rows in Excel with +/- toggles. "
    ...
    if group_count:
        ...
        print(
            f"Applied {group_count} Excel row group(s) by {args.group_by} ({collapse_state})"
        )
